import Autosubst4Lean.Test7.Syntax
...
| .exists_ a0, σ => .exists_ (a0.subst σ.lift)
...
| .pack a0 a1 a2, σ => .pack (a0.subst σ) (a1.subst σ) (a2.subst σ.lift)
...
theorem Exp.weaken_subst_comm {t : Exp (s1 ++ K)} {σ : Subst s1 s2} :
  ...
  | .pack f0 f1 f2 =>
    have ih0 := Ty.weaken_subst_comm (t:=f0) (σ:=σ) (K:=K) (k0:=k0)
    have ih1 := Exp.weaken_subst_comm (t:=f1) (σ:=σ) (K:=K) (k0:=k0)
    have ih2 := Ty.weaken_subst_comm (t:=f2) (σ:=σ) (K:=K,X) (k0:=k0)
    simp [Exp.subst, Exp.rename, ih0, ih1]
    exact ih2
  | .unpack f0 f1 =>
    have ih0 := Exp.weaken_subst_comm (t:=f0) (σ:=σ) (K:=K) (k0:=k0)
    have ih1 := Exp.weaken_subst_comm (t:=f1) (σ:=σ) (K:=K,X,x) (k0:=k0)
    simp [Exp.subst, Exp.rename, ih0]
    exact ih1
  | .pair f0 f1 =>
    have ih0 := Exp.weaken_subst_comm (t:=f0) (σ:=σ) (K:=K) (k0:=k0)
